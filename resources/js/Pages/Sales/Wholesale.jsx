...
import { 
    ...
    Trash2, CornerUpLeft, DollarSign, Gem, ShieldCheck, Building2
} from 'lucide-react';
...
export default function Wholesale({ sales = { data: [] }, branches = [], customers = [], products = [], filters = {} }) {
    ...
    useFilter(route('sales.wholesale'), {
        search,
        status,
        branch_id: branchId
    });
    ...
    const handleDeleteSale = (sale) => {
        if (confirm(`Are you sure you want to delete wholesale invoice #${sale.invoice_no}?`)) {
    ...
    return (
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                            ...
                            {t('wholesales') || 'Wholesales'}
                        </h2>
                        <p className="text-sm text-gray-500 mt-1">{t('Manage wholesale dealer sales, bulk orders, and B2B invoices')}</p>
                    </div>
                    <Link
                        href={route('sales.wholesale.create')}
                        className="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2.5 rounded-xl font-semibold text-sm transition-colors flex items-center gap-2 shadow-md hover:shadow-lg"
                    >
        ...
        >
            <Head title={t('wholesales') || "Wholesales"} />
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex-1 min-w-[200px]">
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="Search wholesale by invoice no, dealer name or phone..."
                                className="w-full pl-9 text-sm rounded-xl border-gray-300 focus:border-amber-500 focus:ring-amber-500"
                            />
                        </div>
                    </div>
                    ...
                    <div className="w-40">
                        <select
                            ...
                            className="w-full text-sm rounded-xl border-gray-300 focus:border-amber-500 focus:ring-amber-500"
                        >
                    ...
                    <table className="w-full text-xs text-left">
                        <thead className="bg-amber-900 text-white font-bold uppercase tracking-wider text-[11px]">
                            <tr>
                                ...
                                <th className="px-3.5 py-3.5">{t('Customer / Dealer')}</th>
                        ...
                        <tbody className="divide-y divide-gray-100 font-medium text-gray-800">
                            {sales.data && sales.data.length > 0 ? (
                                sales.data.map((sale, idx) => {
                                    ...
                                    const statusType = dueAmount <= 0 ? 'paid' : (paidAmount > 0 ? 'partial' : 'due');
                                    ...
                                    return (
                                        <tr key={sale.id} className="hover:bg-amber-50/40 transition-colors">
                                            ...
                                            <td className="px-3.5 py-3 font-bold text-gray-900">
                                                {sale.customer?.name || (isBn ? 'পাইকারি ক্রেতা' : 'Wholesale Client')}
                                            </td>
                                            ...
                                            <td className="px-3.5 py-3 text-center">
                                                <Dropdown>
                                                    ...
                                                    <Dropdown.Content align={sales.data?.length > 3 && idx >= sales.data.length - 2 ? 'top-right' : 'right'} className="w-48 py-1 bg-white text-xs font-bold border border-gray-200 shadow-2xl z-50">
                                                        ...
                                                        {/* Print Invoice */}
                                                        <Link
                                                            href={route('sales.show', [sale.id, { print: 'true' }])}
                                                            className="w-full text-left px-3.5 py-2 text-amber-900 hover:bg-amber-50 flex items-center gap-2 font-bold"
                                                        >
                                                        ...
                                                        >
                                                            <CreditCard className="w-4 h-4 text-emerald-600" /> {t('Record Payment')}
                                                        </button>
                                ...
                                <tr>
                                    <td colSpan="9" className="px-4 py-12 text-center text-gray-400 font-medium">
                                        {t('No wholesale sales records found.')}
                                    </td>
                                </tr>
                            )}
                        </tbody>
                    ...
                    <div className="p-6 bg-white rounded-3xl text-gray-900 space-y-4 font-sans border border-amber-200">
                        <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-base font-black text-gray-900">
                                    Add Wholesale Payment #{paymentSale.invoice_no}
                                </h3>
                            </div>
                            <button
                                type="button"
                        ...
                        <form onSubmit={handlePaymentSubmit} className="space-y-3.5 text-xs font-semibold">
                            <div className="grid grid-cols-2 gap-3 bg-amber-50/60 p-3 rounded-xl border border-amber-200/80">
                                <div>
                                    <span className="text-[10px] text-gray-500 font-bold block uppercase">Dealer / Customer</span>
                                    <span className="font-bold text-gray-900">{paymentSale.customer?.name || 'Wholesale Client'}</span>
                                </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Wholesale payment note..."
                                    className="w-full rounded-xl border-gray-300 text-xs py-2"
                                />
                            </div>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-black tracking-tight text-white flex items-center gap-2">
                                            Wholesale Invoice #{selectedSale.invoice_no}
                                        </h3>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                            ...
                            <div className="p-8 sm:p-10 space-y-8 bg-white text-gray-900 print-hide no-print">
                                <div className="flex flex-col sm:flex-row justify-between items-start border-b-2 border-amber-500/30 pb-6 gap-4">
                                    ...
                                    <div className="text-right bg-amber-50 p-4 rounded-2xl border border-amber-200">
                                        <span className="text-[11px] font-extrabold text-amber-800 uppercase tracking-widest block">WHOLESALE INVOICE</span>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200">
                                        <span className="text-[11px] font-extrabold text-amber-700 uppercase tracking-wider block mb-1">DEALER / CLIENT</span>
                                        <h4 className="text-base font-extrabold text-gray-900">{selectedSale.customer?.name || 'Wholesale Client'}</h4>
                            ...
                            <div className="mini-pos-receipt-print hidden print:block w-[80mm] max-w-[80mm] bg-white p-4 text-xs font-mono text-gray-900 space-y-2.5">
                                ...
                                <div className="text-[10px] space-y-0.5 border-b border-dashed border-gray-400 pb-2">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-bold">{selectedSale.customer?.name || 'Wholesale Client'}</span>
                                    </div>
