                                ...
                                <tbody className="bg-white divide-y divide-gray-100">
                                    {suppliers.data.map((supplier) => (
                                        <tr key={supplier.id} className="hover:bg-gray-50/80 transition-colors">
                                            {/* Supplier: Avatar + Company Name + Code */}
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <div className="flex-shrink-0 h-10 w-10">
                                                        {supplier.photo ? (
                                                            <img className="h-10 w-10 rounded-full object-cover border border-gray-200" src={`/storage/${supplier.photo}`} alt="" />
                                                        ) : (
                                                    ...
                                                    <div className="ml-4">
                                                        <div className="text-sm font-bold text-gray-900">{supplier.company_name || supplier.name}</div>
                                            ...
                                            {/* Address & NID */}
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-gray-700 max-w-[200px] truncate" title={supplier.address}>
                                                    {supplier.address}
                                                </div>
                                                {supplier.nid_number && (
                                                    <div className="text-xs text-gray-400">NID: {supplier.nid_number}</div>
                                                )}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-bold text-red-600">
                                                    BDT {Number(supplier.due_balance || 0).toLocaleString()}
                                                </div>
                                                {supplier.credit_limit > 0 && (
                                                    <div className="text-xs text-gray-500">
                                                        Limit: BDT {Number(supplier.credit_limit).toLocaleString()}
                                                    </div>
                                                )}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <Dropdown>
                                                    ...
                                                    <Dropdown.Content align="right" width="48">
                                                        <Dropdown.Link href={route('suppliers.show', supplier.id)} className="flex items-center text-gray-700">
                                                            <Eye className="w-4 h-4 mr-2 text-indigo-500" /> View Details
                                                        </Dropdown.Link>
                                                        <Dropdown.Link href={route('suppliers.edit', supplier.id)} className="flex items-center text-gray-700">
                                                            <Edit className="w-4 h-4 mr-2 text-blue-500" /> Edit Supplier
                                                        </Dropdown.Link>
