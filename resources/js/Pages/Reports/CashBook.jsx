...
import { 
    Wallet, Printer, TrendingUp, TrendingDown, ArrowDownRight, ArrowUpRight, DollarSign, Filter, RotateCcw
} from 'lucide-react';
...
export default function CashBook({ 
    ...
    useFilter(route('reports.cash-book'), {
    ...
    return (
            ...
            header={
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">{t('cashInHandRegister') || 'Cash in hand receipts, payments, and starting-to-ending balance summary'}</p>
                    </div>
        ...
        >
            <Head title={t('cashBook') || 'Cash Book'} />

            {/* Account Reports Navigation */}
            <AccountReportsNav activeTab="cash-book" />
            ...
            <div className="hidden print:block mb-6 border-b border-gray-300 pb-3">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <h2 className="text-sm font-semibold text-gray-700">{t('cashBook') || 'Cash Book'} (Cash in Hand Statement)</h2>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 mb-6">
                <div className="bg-white border border-gray-100 p-4 rounded-2xl shadow-2xs">
                    <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">{t('openingCash') || 'Opening Cash'}</p>
                    <p className="text-lg font-black text-gray-900 mt-1.5">{fmtMoney(summary.opening_cash)}</p>
                ...
                <div className="bg-white border border-emerald-100 p-4 rounded-2xl shadow-2xs">
                    <div className="flex items-center justify-between">
                        <p className="text-[11px] font-bold text-emerald-800 uppercase tracking-wider">{t('totalCashIn') || 'Total Cash In'}</p>
                        <span className="p-1 rounded-md bg-emerald-50 text-emerald-700">
                            <ArrowDownRight className="w-3.5 h-3.5" />
                        </span>
                    </div>
                    <p className="text-lg font-black text-emerald-700 mt-1.5">{fmtMoney(summary.total_cash_in)}</p>
                    <p className="text-[10px] text-emerald-600 mt-1">{t('receipts') || 'Cash receipts'}</p>
                </div>

                <div className="bg-white border border-rose-100 p-4 rounded-2xl shadow-2xs">
                    <div className="flex items-center justify-between">
                        <p className="text-[11px] font-bold text-rose-800 uppercase tracking-wider">{t('totalCashOut') || 'Total Cash Out'}</p>
                        <span className="p-1 rounded-md bg-rose-50 text-rose-700">
                            <ArrowUpRight className="w-3.5 h-3.5" />
                        </span>
                    </div>
                    <p className="text-lg font-black text-rose-700 mt-1.5">{fmtMoney(summary.total_cash_out)}</p>
                    <p className="text-[10px] text-rose-600 mt-1">{t('payments') || 'Cash disbursements'}</p>
                </div>

                <div className="bg-white border border-amber-100 p-4 rounded-2xl shadow-2xs">
                    <p className="text-[11px] font-bold text-amber-800 uppercase tracking-wider">{t('netCashFlow') || 'Net Cash Flow'}</p>
                    <p className={`text-lg font-black mt-1.5 ${summary.net_cash_flow >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                        {fmtMoney(summary.net_cash_flow)}
                    </p>
                    <p className="text-[10px] text-gray-400 mt-1">{summary.net_cash_flow >= 0 ? 'Net Cash Gain' : 'Net Cash Reduction'}</p>
                </div>

                <div className="bg-white border border-emerald-200 p-4 rounded-2xl shadow-2xs bg-emerald-50/20">
                    <p className="text-[11px] font-bold text-emerald-900 uppercase tracking-wider">{t('closingCash') || 'Closing Cash in Hand'}</p>
                    <p className="text-lg font-black text-emerald-950 mt-1.5">{fmtMoney(summary.closing_cash)}</p>
                    <p className="text-[10px] text-emerald-700 mt-1">{isBn ? `${toBn(transactions.length)} টি ক্যাশ এন্ট্রি` : `${transactions.length} cash entries`}</p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-2xs p-4 pb-24 overflow-x-visible">
                <div className="flex items-center justify-between mb-3">
                    <div>
                        <h3 className="text-sm font-bold text-gray-900">{t('cashBookLedger') || 'Cash Book Register'}</h3>
                    </div>
                    <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200">
                        {isBn ? `ক্যাশ লেনদেন: ${toBn(transactions.length)}` : `Cash Entries: ${transactions.length}`}
                    </span>
                </div>
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                        <thead>
                            <tr className="bg-[#e68a1d] text-white">
                                ...
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap">{t('particulars') || 'Particulars / Description'}</th>
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap text-right text-emerald-100">{t('cashIn') || 'Cash In (Debit)'}</th>
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap text-right text-rose-100">{t('cashOut') || 'Cash Out (Credit)'}</th>
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap text-right">{t('cashBalance') || 'Cash in Hand'}</th>
                            </tr>
                        </thead>
                            ...
                            {transactions.length > 0 ? (
                                transactions.map((tx) => (
                                    <tr key={tx.id} className="hover:bg-gray-50/80 transition">
                                        ...
                                        <td className="px-3.5 py-2.5 text-right font-bold text-gray-900 whitespace-nowrap bg-emerald-50/20">
                                            {fmtMoney(tx.balance)}
                                        </td>
                                    </tr>
                                ))
                                ...
                                <tr>
                                    <td colSpan="7" className="p-8 text-center text-gray-400">
                                        {t('noCashTransactionsFound') || 'No cash transactions found for the selected period.'}
                                    </td>
                                </tr>
                            )}
                        </tbody>
                        {transactions.length > 0 && (
                            <tfoot className="bg-gray-50 border-t-2 border-gray-200 font-bold">
                                <tr>
                                    <td colSpan="4" className="px-3.5 py-2.5 text-right uppercase text-gray-700">
                                        {t('total') || 'Total'}:
                                    </td>
                                    <td className="px-3.5 py-2.5 text-right text-emerald-700 whitespace-nowrap">
                                        {fmtMoney(summary.total_cash_in)}
                                    </td>
                                    <td className="px-3.5 py-2.5 text-right text-rose-700 whitespace-nowrap">
                                        {fmtMoney(summary.total_cash_out)}
                                    </td>
                                    <td className="px-3.5 py-2.5 text-right text-emerald-900 whitespace-nowrap">
                                        {fmtMoney(summary.closing_cash)}
                                    </td>
                                </tr>
                            </tfoot>
                        )}
