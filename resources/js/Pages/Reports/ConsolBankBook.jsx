...
import { 
    Landmark, Printer, TrendingUp, TrendingDown, ArrowDownRight, ArrowUpRight, CreditCard, Filter, RotateCcw
} from 'lucide-react';
import useFilter from '@/Hooks/useFilter';
import { useLanguage } from '@/Context/LanguageContext';
import AccountReportsNav from './Components/AccountReportsNav';

export default function ConsolBankBook({ 
    transactions = [], 
    summary = {}, 
    branches = [], 
    bankAccounts = [], 
    ...
    useFilter(route('reports.consol-bank-book'), {
    ...
    return (
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
                            <Landmark className="w-6 h-6" style={{ color: 'rgb(177, 118, 51)' }} />
                            {t('consolBankBook') || 'Consol Bank Book'}
                        </h2>
                        <p className="text-xs text-gray-500 mt-0.5">{t('consolidatedBankStatements') || 'Consolidated bank deposits, withdrawals, cheques, and balance summary'}</p>
                    </div>
        ...
        >
            <Head title={t('consolBankBook') || 'Consol Bank Book'} />

            {/* Account Reports Navigation */}
            <AccountReportsNav activeTab="consol-bank-book" />
            ...
            <div className="hidden print:block mb-6 border-b border-gray-300 pb-3">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <h2 className="text-sm font-semibold text-gray-700">{t('consolBankBook') || 'Consol Bank Book'} (Consolidated Bank Statement)</h2>
                    </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xs border border-gray-100 p-4 mb-6 print:hidden">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div>
                        <label className="block text-[11px] font-bold text-gray-600 uppercase mb-1">{t('bankAccount') || 'Bank Account'}</label>
                        <select
                            value={accountId}
                            onChange={(e) => setAccountId(e.target.value)}
                            className="w-full text-xs rounded-xl border-gray-200 focus:ring-amber-500 focus:border-amber-500 py-2 px-3"
                        >
                            <option value="">{t('allBankAccounts') || 'All Consolidated Banks'}</option>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 mb-6">
                <div className="bg-white border border-gray-100 p-4 rounded-2xl shadow-2xs">
                    <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">{t('openingBank') || 'Opening Bank'}</p>
                    <p className="text-lg font-black text-gray-900 mt-1.5">{fmtMoney(summary.opening_bank)}</p>
                ...
                <div className="bg-white border border-emerald-100 p-4 rounded-2xl shadow-2xs">
                    <div className="flex items-center justify-between">
                        <p className="text-[11px] font-bold text-emerald-800 uppercase tracking-wider">{t('totalDeposits') || 'Total Deposits'}</p>
                        <span className="p-1 rounded-md bg-emerald-50 text-emerald-700">
                            <ArrowDownRight className="w-3.5 h-3.5" />
                        </span>
                    </div>
                    <p className="text-lg font-black text-emerald-700 mt-1.5">{fmtMoney(summary.total_deposits)}</p>
                    <p className="text-[10px] text-emerald-600 mt-1">{t('bankReceipts') || 'Bank & POS deposits'}</p>
                </div>

                <div className="bg-white border border-rose-100 p-4 rounded-2xl shadow-2xs">
                    <div className="flex items-center justify-between">
                        <p className="text-[11px] font-bold text-rose-800 uppercase tracking-wider">{t('totalWithdrawals') || 'Total Withdrawals'}</p>
                        <span className="p-1 rounded-md bg-rose-50 text-rose-700">
                            <ArrowUpRight className="w-3.5 h-3.5" />
                        </span>
                    </div>
                    <p className="text-lg font-black text-rose-700 mt-1.5">{fmtMoney(summary.total_withdrawals)}</p>
                    <p className="text-[10px] text-rose-600 mt-1">{t('bankPayments') || 'Cheques & payments'}</p>
                </div>

                <div className="bg-white border border-amber-100 p-4 rounded-2xl shadow-2xs">
                    <p className="text-[11px] font-bold text-amber-800 uppercase tracking-wider">{t('netBankMovement') || 'Net Bank Flow'}</p>
                    <p className={`text-lg font-black mt-1.5 ${summary.net_bank_flow >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                        {fmtMoney(summary.net_bank_flow)}
                    </p>
                    <p className="text-[10px] text-gray-400 mt-1">{summary.net_bank_flow >= 0 ? 'Net Bank Growth' : 'Net Bank Decrease'}</p>
                </div>

                <div className="bg-white border border-blue-200 p-4 rounded-2xl shadow-2xs bg-blue-50/20">
                    <p className="text-[11px] font-bold text-blue-900 uppercase tracking-wider">{t('closingBank') || 'Closing Bank Balance'}</p>
                    <p className="text-lg font-black text-blue-950 mt-1.5">{fmtMoney(summary.closing_bank)}</p>
                    <p className="text-[10px] text-blue-700 mt-1">{isBn ? `${toBn(transactions.length)} টি ব্যাংক এন্ট্রি` : `${transactions.length} bank entries`}</p>
                </div>
            </div>

            {/* Bank Book Table Card */}
            <div className="bg-white rounded-2xl border border-gray-100 shadow-2xs p-4 pb-24 overflow-x-visible">
                <div className="flex items-center justify-between mb-3">
                    <div>
                        <h3 className="text-sm font-bold text-gray-900">{t('consolBankBookLedger') || 'Consolidated Bank Statement'}</h3>
                    </div>
                    <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-800 border border-blue-200">
                        {isBn ? `ব্যাংক রেকর্ড: ${toBn(transactions.length)}` : `Bank Entries: ${transactions.length}`}
                    </span>
                </div>
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                        <thead>
                            <tr className="bg-[#e68a1d] text-white">
                                ...
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap">{t('voucherNo') || 'Voucher / Cheque'}</th>
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap">{t('type') || 'Type'}</th>
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap">{t('particulars') || 'Particulars / Beneficiary'}</th>
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap">{t('channel') || 'Channel'}</th>
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap text-right text-emerald-100">{t('depositIn') || 'Deposit (Debit)'}</th>
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap text-right text-rose-100">{t('withdrawalOut') || 'Withdrawal (Credit)'}</th>
                                <th className="px-3.5 py-2.5 text-[11px] font-bold uppercase whitespace-nowrap text-right">{t('bankBalance') || 'Bank Balance'}</th>
                            </tr>
                        </thead>
                            ...
                            {transactions.length > 0 ? (
                                transactions.map((tx) => (
                                    <tr key={tx.id} className="hover:bg-gray-50/80 transition">
                                        ...
                                        <td className="px-3.5 py-2.5 whitespace-nowrap">
                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-50 text-blue-700 border border-blue-200 capitalize">
                                        ...
                                        <td className="px-3.5 py-2.5 text-right font-bold text-gray-900 whitespace-nowrap bg-blue-50/20">
                                            {fmtMoney(tx.balance)}
                                        </td>
                                    </tr>
                                ))
                                ...
                                <tr>
                                    <td colSpan="8" className="p-8 text-center text-gray-400">
                                        {t('noBankTransactionsFound') || 'No bank transactions found for the selected period.'}
                                    </td>
                                </tr>
                            )}
                        </tbody>
